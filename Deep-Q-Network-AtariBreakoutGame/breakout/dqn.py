...
def test(model):
    ...
    while True:
        ...
        if terminal:
            print(f"Game Over! Total bricks destroyed: {game_state.bricks_destroyed}")
            break
...
def main(mode):
    if mode == 'test':
        model = torch.load('trained_model/current_model_260000.pth', map_location='cpu').eval()
        test(model)
    elif mode == 'train':
        ...
        model = torch.load('trained_model/current_model_0000.pth', map_location='cpu').eval()
        start = time.time()
        train(model, start)
